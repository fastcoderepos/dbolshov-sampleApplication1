...
public class CustomerControllerTest {
    ...
    public void CreateCustomer_CustomerDoesNotExist_ReturnStatusOk() throws Exception {
        ...
        customerInput.setStoreId(Integer.parseInt(store.getStoreId().toString()));
    ...
    @Test
    public void CreateCustomer_storeDoesNotExists_ThrowEntityNotFoundException() throws Exception {
        CreateCustomerInput customer = createCustomerInput();
        ObjectWriter ow = new ObjectMapper()
            .registerModule(new JavaTimeModule())
            .disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS)
    ...
    public void UpdateCustomer_CustomerExists_ReturnStatusOk() throws Exception {
        ...
        customerInput.setActive(entity.getActive());
    ...
    public void GetStore_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
        org.assertj.core.api.Assertions
            .assertThatThrownBy(
                () ->
                    mvc
                        .perform(get("/customer/999/store").contentType(MediaType.APPLICATION_JSON))
                        .andExpect(status().isOk())
            )
            .hasCause(new EntityNotFoundException("Not found"));
    }
    ...
    public void GetStore_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        mvc
            .perform(get("/customer/" + customer.getCustomerId() + "/store").contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    }
}
