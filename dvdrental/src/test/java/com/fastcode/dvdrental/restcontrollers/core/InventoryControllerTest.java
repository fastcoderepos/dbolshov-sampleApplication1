...
public class InventoryControllerTest {
    ...
    public void CreateInventory_InventoryDoesNotExist_ReturnStatusOk() throws Exception {
        ...
        inventoryInput.setStoreId(Integer.parseInt(store.getStoreId().toString()));
    ...
    @Test
    public void CreateInventory_storeDoesNotExists_ThrowEntityNotFoundException() throws Exception {
        CreateInventoryInput inventory = createInventoryInput();
        ObjectWriter ow = new ObjectMapper()
            .registerModule(new JavaTimeModule())
            .disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS)
    ...
    public void GetStore_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
        org.assertj.core.api.Assertions
            .assertThatThrownBy(
                () ->
                    mvc
                        .perform(get("/inventory/999/store").contentType(MediaType.APPLICATION_JSON))
                        .andExpect(status().isOk())
            )
            .hasCause(new EntityNotFoundException("Not found"));
    }
    ...
    public void GetStore_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        mvc
            .perform(get("/inventory/" + inventory.getInventoryId() + "/store").contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    }
}
